...
AI scores movement candidates by distance, terrain, exposure, attack value, kill value, counter-damage risk, role shaping and objective pressure. It also evaluates light-tank fire-support marks when a same-faction follow-up attacker can use the suppression bonus. Hard difficulty enables a one-ply lookahead against visible player retaliation. `tools/ai_trace_report.gd` regenerates `docs/progress/ai_trace_report.md` from the live `AIController.plan_trace_for_unit()` diagnostics, including primary/secondary objective score splits and fire-support mark scores.
