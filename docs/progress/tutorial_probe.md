...
| tut_04_armor_at_veteran_general | 9 | movement (3 units can move), attack (教學 AT->Pz.IV 目標, M10 教學車->Pz.IV 目標), counterattack (adjacent enemy pair), armor (armored units present), anti_armor (player AT weapon present), armor_standoff (M10 教學車->Pz.IV 目標 at 2), general_skill (player general), veteran (player veteran), deployment_locked | none |
